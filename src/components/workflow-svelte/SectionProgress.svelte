  ...
  function selectStep(step: Step) {
    ...
    stateStore.update(state => {
      const newState = {
        ...state,
        currentStep: step,
        // Ensure we keep the current section and component
        currentSection: currentSection,
        currentComponent: 'Workflow'
      };
      console.log('New state after step selection:', newState);
      return newState;
    });
        ...
        >
          <a 
            ...
            class={`group flex w-full items-center ${
              index <= currentSection.steps.indexOf(currentStep) ? 'text-primary-dark' : 'text-primary-dark'
            }`}
          >
            <span class="flex items-center px-6 py-4 text-sm font-medium">
              <span class={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${
                index < currentSection.steps.indexOf(currentStep) ? 'bg-primary-dark' : 
                (index === currentSection.steps.indexOf(currentStep) ? 'bg-white border-2 border-primary-dark' : 'border-2 border-primary-dark')
              }`}>
