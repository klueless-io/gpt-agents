  ...
  <nav aria-label="Progress">
    <ol role="list" class="divide-y divide-gray-600 rounded-md border border-gray-300 md:flex md:divide-y-0 bg-primary-light">
      {#each currentSection.steps as step, index}
        <li class="relative md:flex md:flex-1" data-key={step.name}>
          <a href="#" class={`group flex w-full items-center ${step === currentStep ? 'text-primary-dark' : 'text-primary-dark'}`}>
            <span class="flex items-center px-6 py-4 text-sm font-medium">
              <span class={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${step === currentStep ? 'bg-white border-2 border-primary-dark' : 'bg-primary-dark'}`}>
